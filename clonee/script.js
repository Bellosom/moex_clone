const futuresData = {
    indices: [
        ...
        {code: 'CNI', name: 'Индекс Потребительского сектора'},
...
function loadChart(futureCode, chartNumber) {
    ...
    if (chartCache[cacheKey]) {
        ...
        fetch(`http://www.openinterest.ru/api/get_chart_data?futureCode=${encodeURIComponent(futureCode)}&chartNumber=${chartNumber}`)
            .then(response => {
                if (!response.ok) {
